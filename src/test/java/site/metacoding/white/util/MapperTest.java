...
public class MapperTest {
    ...
    public void convert_test2() {
        ...
        // List<Dog> -> List<DogDto>
        List<DogDto> dogDtoList = dogList.stream() // stream()은 최상위 타입을 벗겨버림. list 타입 안의 dog 타입 2개의 구조였으나 stream을 사용함으로써
                                                   // list타입이 벗겨짐.
                .map((dog) -> new DogDto(dog.getId(), dog.getName()))// .map의 경우 타입을 변환시켜주는 것. dog->dogDto로 변경함
                .collect(Collectors.toList());// 변경된 타입을 list화 시켜줄 때 사용
